...
export default class Scores extends React.Component {
  ...
  fetchForDate(date) {
    ...
    return (
      fetch(
        `https://ca.global.nba.com/stats2/scores/daily.json?countryCode=CA&gameDate=${date}&locale=en&tz=${tz}`
      )
        ...
        .then(responseJson => {
          ...
          let games = [];
          if (
            responseJson.payload.date &&
            responseJson.payload.date.games.length > 0
          ) {
